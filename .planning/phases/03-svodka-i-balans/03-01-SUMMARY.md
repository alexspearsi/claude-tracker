...
**Сценарий 6 (ошибка/авторизация сводки) не прогнан вживую** — потребовал бы намеренной порчи access-куки или временной остановки только `/transactions/summary`, что рискованно на общем dev-стенде между фазами. Вместо этого подтверждён код-ревью: `loadMonthlySummary` структурно идентичен уже многократно проверенному в фазе 2 паттерну `loadRecentTransactions` (тот же `ApiError.status === 401` → `unauthorized`, тот же независимый `error`-текст без общего `catch` на обе ветки `DashboardView`). Остаётся зафиксированным как открытый пункт для полного финального прогона фазы, если он потребуется.
...
- 5 из 6 ручных сценариев `03-VALIDATION.md` подтверждены визуально после мержа (см. D1/D2/D4 `coverage`)
- Сценарий ошибки/авторизации сводки подтверждён только код-ревью (структурная идентичность с проверенным паттерном фазы 2) — не критично для мержа, но стоит держать в уме на будущее
- Фаза 3 готова к финальному verify-work и мержу в `master`

...
coverage:
  ...
  - id: D4
    ...
    verification:
      - kind: manual_procedural
        ref: "браузерная UAT после мержа в feature/transactions-crud (bugcheck@test.local, http://localhost:3001/expenses): панель «Период»/«Тип»/«Категория» видна сразу над таблицей без сворачивания; после фильтра, обнулившего выдачу, панель осталась на месте над состоянием «Ничего не найдено»"
        status: pass
    human_judgment: true
  - id: D5
    ...
    verification:
      - kind: manual_procedural
        ref: "та же сессия: выбор «Доход» в Тип -> URL ?type=INCOME без page; выбор категории «Продукты» -> ?categoryId=<uuid>; выбор диапазона 1–21 сентября -> добавились dateFrom=2026-09-01&dateTo=2026-09-21, транзакция от 21.09 осталась в выдаче (инклюзивная граница dateTo подтверждена и на живом клиенте, не только curl'ом из 02-RESEARCH Pitfall 2)"
        status: pass
    human_judgment: true
  - id: D6
    ...
    verification:
      - kind: manual_procedural
        ref: "та же сессия: фильтр type=INCOME (нет доходов у тестового пользователя) показал ровно «Ничего не найдено» / «Попробуйте изменить период, тип или категорию», без кнопки «Добавить транзакцию» в теле состояния"
        status: pass
    human_judgment: true
  - id: D7
...
**Human-check обеих задач не выполнен вживую в браузере исполнителем плана.** В его сессии не было инструмента браузерной автоматизации. Вместо визуальной проверки:
...
**Дополнено после мержа в `feature/transactions-crud`.** Координатор прошёл визуальную UAT в браузере (`http://localhost:3001/expenses`, `bugcheck@test.local`) и при этом нашёл реальный баг, не пойманный ни typecheck/lint, ни curl-верификацией:

**Найден и исправлен: рассинхронизация отображаемого значения `Select` после сброса фильтра.**
- **Found during:** визуальная UAT, шаг «выбрать Тип → Доход → снова открыть → выбрать Все»
- **Issue:** оба `Select` (`Тип`, `Категория`) в `transaction-filters.tsx` были `value={filters.type}`/`value={filters.categoryId}` без `key`. Radix `Select` не сбрасывает отображаемое значение триггера при переходе controlled `value` с реальной строки на `undefined` (переключение controlled/uncontrolled на лету не поддерживается) — после выбора «Все» URL и данные обновлялись верно, но триггер продолжал показывать последний выбранный пункт («Доход») до полной перезагрузки страницы.
- **Fix, попытка 1:** добавлен `key={filters.type ?? 'all'}`/`key={filters.categoryId ?? 'all'}`, чтобы форсировать remount при сбросе. Это вскрыло вторую, более серьёзную проблему: оба `Select` — соседи в одном родителе, и при одновременном отсутствии обоих фильтров у них совпадал `key="all"` — React выбросил `Console Error: Encountered two children with the same key 'all'` и оба триггера начали показывать одно и то же значение («Доход» дважды).
- **Fix, попытка 2 (финальная):** ключи сделаны уникальными по полю — `key={\`type-${filters.type ?? 'all'}\`}` и `key={\`category-${filters.categoryId ?? 'all'}\`}`. Повторная UAT: тип «Доход» → «Все» корректно возвращает плейсхолдер «Тип», категория «Продукты» → диапазон периода — оба контрола сброшены независимо, ошибок в консоли Next.js (`1 Issue` индикатор) больше нет.
- **Files modified:** `apps/web/src/widgets/expenses-list/ui/transaction-filters.tsx`
- **Verification:** `npm run typecheck` и `npm --prefix apps/web run lint` зелёные после каждой правки; полный повторный проход UAT (тип, категория, период — каждый выбран и сброшен по отдельности и в комбинации) без визуальных и консольных ошибок.
- **Committed separately** координатором поверх мержа (см. историю `feature/transactions-crud` после мержа плана 02-04).
...
- TXN-01..06 — весь скоуп фазы 2 — реализован в коде и подтверждён визуально; фаза готова к финальной приёмке (план 02-05)
- Серверный контракт фильтров (все четыре параметра, их комбинации, инклюзивность последнего дня периода) подтверждён и напрямую, и через живой браузер
- Визуальный UAT для D4/D5/D6 этого плана пройден координатором после мержа (`status: pass`); попутно найден и исправлен реальный баг рассинхронизации `Select`-триггеров (см. Issues Encountered) — этот класс проблем (controlled-компонент с Radix, сброс значения на `undefined`) стоит держать в уме для будущих фильтров/форм с похожим паттерном
- Визуальный чекпоинт формы добавления транзакции на `/dashboard` из плана 02-01 закрыт координатором ранее (см. `02-01-SUMMARY.md`, `02-03-SUMMARY.md`)

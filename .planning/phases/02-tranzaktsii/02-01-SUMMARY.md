  ...
  - id: D1
    ...
    verification:
      - kind: manual_procedural
        ref: "визуальная проверка заблокирована аномалией окружения (см. Issues Encountered) — логика подтверждена через прямые вызовы API"
        status: unknown
    human_judgment: true
    rationale: "Браузерная проверка списка транзакций на /dashboard заблокирована средовой аномалией этого worktree (GET /transactions 400) — сама форма и создание транзакции не задействуют этот путь и подтверждены отдельно через curl"
  - id: D2
    description: "Созданная транзакция появляется в списке без перезагрузки (revalidatePath), сумма визуализируется цветом+знаком"
    requirement: "TXN-05"
    verification:
      - kind: manual_procedural
        ref: "не проверено визуально из-за среды — см. Issues Encountered"
        status: unknown
    human_judgment: true
    rationale: "Требует рабочего /dashboard списка транзакций, который в этом worktree недоступен; рекомендуется перепроверить на чистой среде или после мержа"
  - id: D3
...
**Рекомендация:** перепроверить визуально сценарий TXN-01/TXN-05 (человеческий чекпоинт из плана) на чистой среде — либо после мержа веток фазы в общую ветку, либо в основном рабочем каталоге, а не в этом worktree.
...
- **Не выполнена:** визуальная браузерная UAT-проверка формы (human-check из плана) — заблокирована средовой аномалией, описанной выше. Логика подтверждена на уровне API-контракта, но финальная визуальная приёмка должна пройти на чистой среде перед завершением фазы (естественная точка — план 02-05, полный приёмочный проход)

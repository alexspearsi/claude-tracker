...
### Срез сводки (только чтение)

`widgets/monthly-summary` читает уже существующий `GET /transactions/summary?month=&year=`
(api этой фазой не меняется, реализован в фазе 2) через один серверный лоадер
`loadMonthlySummary` с простым `try/catch` — в отличие от двухзапросного
...
(групповая выборка на api джойнит категорию сама). Месяц/год вычисляются на сервере из
`now.getUTCMonth() + 1`/`now.getUTCFullYear()` (не локальное время) — api считает границы
месяца через `Date.UTC`, локальное время процесса может разойтись на стыке месяца.

Виджет — одна `Card` на `/dashboard`, подключён перед `RecentTransactions` (сводка — общая
картина, читается первой). Внутри: строка из трёх показателей (Баланс/Доходы/Расходы) и
таблица разбивки по категориям под разделителем. Баланс намеренно **не имеет** цветовой
семантики — нейтральный текст даже при отрицательном значении, в отличие от `TransactionAmount`
(доход/расход), у которого цвет несёт смысл. `TransactionAmount`/`CategoryDot` переиспользованы
напрямую в строках таблицы категорий; React-ключ строки — `` `${categoryId}-${type}` ``, не
голый `categoryId` — групповая выборка `groupBy(['categoryId', 'type'])` на api может дать две
строки с одним и тем же `categoryId`, если категория использована и как доход, и как расход в
одном месяце.

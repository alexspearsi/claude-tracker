...
Query-параметры (все опциональны, `forbidNonWhitelisted` — любой неизвестный ключ отвергается
целиком с 400, а не молча игнорируется):

| Параметр | Формат | Описание |
|---|---|---|
| `dateFrom` | ISO 8601 (строгий) | нижняя граница `date`, включительно |
| `dateTo` | ISO 8601 (строгий) | верхняя граница `date`, включительно — если передан без времени (`YYYY-MM-DD`), граница включает весь этот календарный день целиком, а не только полночь; с полным таймстампом — включает ровно эту миллисекунду |
| `type` | `INCOME` \| `EXPENSE` | фильтр по типу |
| `categoryId` | UUID | фильтр по категории |
| `limit` | целое, 1–100 | размер страницы, по умолчанию 20 |
| `offset` | целое, ≥0 | смещение страницы, по умолчанию 0 |

`200` → `{ items: TransactionDto[], total: number }` (не голый массив — `total` считается по тем
же фильтрам, что и `items`, нужен для пагинации), сортировка `date desc, createdAt desc` с
`createdAt` как вторичным ключом (без него `limit`/`offset` дают дубли/пропуски между
страницами при совпадающей `date`).
...
Тело — любое подмножество полей `POST` (все опциональны). Семантика поля `description`:
**отсутствующий ключ** в теле оставляет прежнее значение без изменений, а **явный `null`**
стирает его в `null` — единственный способ сброса через `UpdateTransactionDto`
(`@IsOptional()`, пропускает и `undefined`, и `null`). Фронт (`updateTransactionAction`)
отправляет `null` именно тогда, когда пользователь стирает описание в форме (пустой `trim()`
→ `null`, а не пустая строка). `200` → обновлённая `TransactionDto`. `404`, если транзакция или
(при передаче) новая категория не найдены/чужие.

...
и список последних 10 транзакций с пагинацией через `?page=` (`widgets/recent-transactions`).
...
и `isGuestOnly` в `proxy.ts` ведут на `/dashboard`, а не на `/expenses`. Страницы категорий и
самих транзакций (`/expenses`) — по-прежнему заглушки без данных, `prisma/seed.ts` не заполнен.
Планы по выполненным задачам — в `.claude/plans/`.
